const { encode } = require('./int-encode.js');
 ...
 * @param {Boolean} options.preventEncodingNumbers - Prevent numbers being converted to shorter utf-8 strings?
...
const parseProp = (prop, val, options) => {
    if (prop.type === 'array') {
        ...
        if (!options.preventEncodingNumbers && prop.type === 'integer' && val >= 1000) return encode(val);
        else if (!options.preventShorteningBooleans && prop.type === 'boolean') return val ? 1 : 0
        return val;
    }
};
